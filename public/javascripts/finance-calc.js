...
function calculate () {
	...
	if (isFinite(monthly)) {
		...
		payment.innerHTML = monthly
			.toFixed(2)
			.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ');
		total.innerHTML = (monthly * payments)
			.toFixed(2)
			.replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ');
...
function chart (principal, interest, monthly, payments) {
	...
	g.fillStyle = '#01751a'; // Now use green paint
	...
	g.fillStyle = '#333'; // Switch to black text
	...
	for (var year = 1; year * 12 <= payments; year++) {
		...
		if (
			year % 5 == 0 &&
			year * 12 !== payments // Number every 5 years
		)
